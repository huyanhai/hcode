<template>
  <div class="flex flex-col h-full gap-2 pb-4 max-w-[800px] mx-auto">
    ...
    <p v-if="errorMessage" class="text-xs text-destructive px-2">
      {{ errorMessage }}
    </p>
  </div>
</template>
...
const { messages, profiles, busy, errorMessage, approvals } =
  storeToRefs(agent);
//#region Props
//#endregion
//#region Emits
...
async function submit() {
  ...
  if (busy.value || !submitData.content.trim()) return;
  try {
    if (!submitData.model) submitData.model = profiles.value[0]?.id ?? "";
    await agent.send(
      submitData.content.trim(),
      submitData.model,
      submitData.fullAccess,
    );
  } catch {
    /* store 已保存可展示错误 */
  }
...
async function answerApproval(approvalId: string, approved: boolean) {
  try {
    await agent.answerApproval(approvalId, approved);
  } catch {
    /* store 已保存可展示错误 */
  }
}
