  ...
  <div class="flex flex-col h-full">
    <div class="pt-14 px-2 flex h-full flex-col gap-4 flex-1">
      ...
      <div class="min-h-0 flex-1">
        <FoldMenus />
        <div
          class="text-sm text-muted-foreground flex gap-1 items-center mb-2 px-2"
        >
          最近
          <ChevronDown :size="16" />
        </div>
        <div class="flex max-h-full flex-col gap-1 overflow-y-auto">
          <Button
            ...
            :key="session.id"
            class="menu-button justify-start shadow-none"
            :variant="session.id === currentSessionId ? 'secondary' : 'ghost'"
            @click="agent.selectSession(session.id)"
          >
            <span class="truncate">{{ session.title }}</span>
          </Button>
          <p v-if="!sessions.length" class="px-2 text-xs text-muted-foreground">
            当前工作区还没有会话。
          </p>
        </div>
      </div>
